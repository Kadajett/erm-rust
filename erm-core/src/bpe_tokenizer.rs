...
impl BpeTokenizer {
    ...
    pub fn train(corpus: &str, num_merges: usize) -> Self {
        ...
        let mut chars: Vec<char> = corpus.chars().collect::<std::collections::HashSet<_>>()
            .into_iter()
            .collect();
    ...
    pub fn load(path: &str) -> ErmResult<Self> {
        let json = std::fs::read_to_string(path)
            .map_err(|e| ErmError::InvalidConfig(format!("cannot read BPE vocab from {path}: {e}")))?;
        Self::from_json(&json)
    }
}
    ...
    fn decode_text(&self, ids: &[u32]) -> String {
        ...
        for &id in ids {
            ...
            if let Some(tok) = self.id_to_token.get(&id) {
                // Strip end-of-word sentinel and replace with space.
